...
class Feeportal extends Component {
  constructor(){
    super();
...
onPayNowClick=(item)=>{
  this.props.history.push(
    {
      pathname: '/student/payment/paynow',
      state: {payment:item,
...
onInstallmentClick=(item)=>{
  this.props.history.push(
    {
      pathname: '/student/payment/installment',
      state: {payment:item,
               student:this.state.studentdetail}
    });
}
...
componentDidMount=()=>{
  ...
  var formdata = new FormData();
        formdata.append('page_name','student-payment');
       var requestOptions = {
           method: 'POST',
           body: formdata,
           redirect: 'follow',
  ...
  var school;
  axios.get('https://zinedu-main.herokuapp.com/users/get-student-details/?username='+this.state.student,requestOptions)
  .then(response =>{console.log(response.data)
      const pack=response.data;
      school=response.data.school_assoc.id;
      this.setState({
          studentdetail:pack,
          
      });
      axios.get('https://zinedu-main.herokuapp.com/payment/get-section-details/?school='+school,requestOptions)
      .then(response =>{console.log(response.data)
          const pack=response.data;
          this.setState({
              package:pack,
...
color: '#000000'
    ...
    var card1=this.state.package.map((item) => (<div style={column} key={item.id}><div class={Styles.box1} ><div ><h1 > &#x20b9;  {item.fee_amount}</h1><br/><p style={{fontFamily: 'Montserrat',fontStyle: 'normal',fontWeight: 'normal',fontSize: '15px',lineHeight: '191.6%', textAlign: 'center' ,color: '#000000'}}>{item.desc}</p><br/><p style={{fontFamily: 'Montserrat',fontStyle: 'normal',fontWeight: 'normal',fontSize: '20px',lineHeight: '191.6%', textAlign: 'center' ,color: '#000000'}}>{item.validity} month</p>{(item.is_instrallment_applicable=='No')?(<button id={Styles.btn3} onClick={()=>{this.onPayNowClick(item)}}>Pay Now</button>):(<div><button id={Styles.btn3} onClick={()=>{this.onPayNowClick(item)}}>Pay Now</button><button id={Styles.btn3} onClick={()=>{this.onInstallmentClick(item)}}style={{margin:'0px 0px 0px 10px'}}> Check Installments</button></div>)}</div></div></div>
      ))
      var card2=this.state.package.map((item) => (<div class={Styles.box1} key={item.id} ><div ><h1 > &#x20b9;  {item.fee_amount}</h1><br/><p style={{fontFamily: 'Montserrat',fontStyle: 'normal',fontWeight: 'normal',fontSize: '15px',lineHeight: '191.6%', textAlign: 'center' ,color: '#000000'}}>{item.desc}</p><br/><p style={{fontFamily: 'Montserrat',fontStyle: 'normal',fontWeight: 'normal',fontSize: '20px',lineHeight: '191.6%', textAlign: 'center' ,color: '#000000'}}>{item.validity} month</p>{(item.is_instrallment_applicable=='No')?(<button id={Styles.btn3} onClick={()=>{this.onPayNowClick(item)}}>Pay Now</button>):(<div><button id={Styles.btn3} onClick={()=>{this.onPayNowClick(item)}}>Pay Now</button><button id={Styles.btn3} onClick={()=>{this.onInstallmentClick(item)}}style={{margin:'0px 0px 0px 10px'}}> Check Installments</button></div>)}</div></div>
      ))
