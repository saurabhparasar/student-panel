...
class TimePerQuestion extends Component{
  constructor(){
    super();
...
console.log(chartdata);
      {this.state.questions.map((item, index)=>{if(item.end_time!=null && item.start_time!=null){
        let endtime =  item.end_time.split('T')[1];
        let starttime =  item.start_time.split('T')[1];
        ...
        let finaltime = finalend - finalstart;
        if(finaltime<300 && finaltime>=0){
          chartdata.options.xaxis.categories.push(index+1);
          chartdata.series[0].data.push(finaltime);
        }
        else{
          chartdata.options.xaxis.categories.push(index+1);
      ...
      var chart;
      var number_questions = this.state.questions.length*100;
        chart = (<div style={{overflowX:'auto', overflowY:'hidden'}}>
          <Chart
                options={chartdata.options}
                series={chartdata.series}
                type="area"
                width={number_questions}
                height="500px"
              />
        </div>)
}
   ...
   var loader={
    ...
    return(
      ...
      <nav id={Styles.nav}>
          <div style={title}>
            <p style={text2}>Time Spent Per Question</p>
          </div>
          <div style={icons}>
  ...
  <path d="M15.571 19.2361C15.8125 19.5104 16.1091 19.7307 16.4415 19.8827C16.7739 20.0347 17.1346 20.1149 17.5 20.1181C17.8665 20.1209 18.2291 20.0434 18.5624 19.891C18.8956 19.7386 19.1915 19.5151 19.429 19.2361H15.571Z" fill="white"/>
        ...
        <div class="gap" style={{marginTop:'60px', padding:'15px'}}>
{chart}
